...
def test_int_division():
    calc = Calculation(operation="IntDivision", operand1=Decimal("7"), operand2=Decimal("2"))
...
def test_int_division_by_zero():
    with pytest.raises(OperationError, match="Integer division by zero is not allowed"):
        Calculation(operation="IntDivision", operand1=Decimal("10"), operand2=Decimal("0"))
...
def test_percentage_by_zero():
    with pytest.raises(OperationError, match="Percentage by zero is not allowed"):
